...
app.get('/categories', (req,res) => {
  ...
  connection.query(
    'SELECT categories.id, categories.name, SUM(items.price) FROM items JOIN expenses ON expenses.id=items.expense_id JOIN categories ON categories.id=items.category_id GROUP BY categories.name, categories.id', (err, results) => {
      if (err) {
        console.error(err)
      } else {
        res.json(results)
